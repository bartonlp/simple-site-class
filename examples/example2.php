...
// The framework uses jQuery which is automatically loaded by SimpleSiteClass.
// If you have cloned this repository from GitHub.com/bartonlp/simple-site-class then put it into
// you /var/www/ directory and use simple-autoload.php.
...
if(empty($home)) {
  echo "<h1>You Must Fill in <b>\$home</b>!</h1><p>Open the example file and fill in the <b>\$home</b> variable.<br>".
      ...
      "For example, the query string would be <b>?home=/var/www</b>, if you cloned the repository into your <b>/var/www</b> direcyory.</p>";
  exit();
}
/* Options for running the repository examples */
// The following will load the simple-autoload.php file, which is probably what you cloned.
$_site = require_once "$home/simple-site-class/includes/simple-autoload.php";
...
//$_site = require_once "$home/vendor/bartonlp/simple-site-class/includes/simple-siteload.php";
/* End of special code */
...
// We can use SimpledbTables to simplify making a table.

$T = new SimpledbTables($S);

$sql = "select site, ip, agent, count, created, lasttime from logagent order by lasttime desc limit 100";

// 'attr' has the table id and border. It could also have a class etc.
// This normally returns an array (see the source code dbTables.class.php) but we only want the zero item which is the
// full table.
...
<p>This page uses SimpledbTables to get the information from the <b>logagent</b> table limit 100.</p>
$tbl
<hr>
